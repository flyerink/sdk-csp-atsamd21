...
void static SERCOM_USART_ErrorClear ( SAMD2x_UART_T *uart )
{
...
void SERCOM_USART_Initialize ( SAMD2x_UART_T *uart )
{
    sercom_registers_t *usart_regs = uart->sercom_reg;
...
uint32_t SERCOM_USART_FrequencyGet ( SAMD2x_UART_T *uart )
{
    sercom_registers_t *usart_regs = uart->sercom_reg;
...
bool SERCOM_USART_SerialSetup ( SAMD2x_UART_T *uart, usart_serial_setup_t *serialSetup, uint32_t clkFrequency )
{
    ...
    if ((serialSetup != NULL) & (serialSetup->baudRate != 0)) {
        if (clkFrequency == 0) {
            clkFrequency = SERCOM_USART_FrequencyGet (uart);
        }
...
uint32_t SERCOM_USART_ErrorGet ( SAMD2x_UART_T *uart )
{
    sercom_registers_t *usart_regs = uart->sercom_reg;
...
void SERCOM_USART_TransmitterEnable ( SAMD2x_UART_T *uart )
{
    sercom_registers_t *usart_regs = uart->sercom_reg;
...
void SERCOM_USART_TransmitterDisable ( SAMD2x_UART_T *uart )
{
    sercom_registers_t *usart_regs = uart->sercom_reg;
...
void SERCOM_USART_ReceiverEnable ( SAMD2x_UART_T *uart )
{
    sercom_registers_t *usart_regs = uart->sercom_reg;
...
void SERCOM_USART_ReceiverDisable ( SAMD2x_UART_T *uart )
{
    sercom_registers_t *usart_regs = uart->sercom_reg;
...
void SERCOM_USART_InterruptDisable (SAMD2x_UART_T *uart )
{
    sercom_registers_t *usart_regs = uart->sercom_reg;

    /* Disable RX interrupt. */
    /* Disable the RX Complete Interrupt */
    usart_regs->USART_INT.SERCOM_INTENCLR = SERCOM_USART_INT_INTENCLR_RXC_Msk;
    usart_regs->USART_INT.SERCOM_INTENCLR = SERCOM_USART_INT_INTENSET_ERROR_Msk;
    usart_regs->USART_INT.SERCOM_CTRLB &= ~SERCOM_USART_INT_CTRLB_RXEN_Msk;
    NVIC_DisableIRQ (uart->vector);
}

void SERCOM_USART_InterruptEnable (SAMD2x_UART_T *uart )
{
    sercom_registers_t *usart_regs = uart->sercom_reg;
...
void static SERCOM_USART_ISR_ERR_Handler ( SAMD2x_UART_T *uart )
{
    sercom_registers_t *usart_regs = uart->sercom_reg;
    uint32_t errorStatus = USART_ERROR_NONE;
...
static void uart_int_cb (SAMD2x_UART_T *uart)
{
    ...
    if (usart_regs->USART_INT.SERCOM_INTENSET != 0) {
        ...
        if ((usart_regs->USART_INT.SERCOM_INTFLAG & SERCOM_USART_INT_INTFLAG_ERROR_Msk) == SERCOM_USART_INT_INTFLAG_ERROR_Msk) {
            SERCOM_USART_ISR_ERR_Handler (uart);
        }
    }
}
...
int hw_usart_init (void)
{
    ...
    SERCOM_USART_Initialize (&uart3);

    uart3.out_rb = ringbuffer_create (UART_RB_BUFSZ);
    uart3.in_rb = ringbuffer_create (UART_RB_BUFSZ);
#endif
